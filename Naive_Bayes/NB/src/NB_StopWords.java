...
public class NB_StopWords {
    ...
    Set<String> vStopSet = new HashSet<>();
            ...
            for(String str : line.split(" ")){
                ...
                if (flag == false){
                    if(filePath.contains("train"))
                        filterStr.add(str);
                    		if(stopFlag)
                    			if(vStopSet.contains(str))
                    				filterStr.remove(str);
                    else {
                        filterStr.add(str);
                		if(stopFlag)
                			if(vStopSet.contains(str))
    ...
    public void createStopList(String path) throws IOException{
        BufferedReader br = null;
        String line = "";
        try {
        	br = new BufferedReader(new FileReader(path));
        	while ((line = br.readLine()) != null) {
                String[] trainingStr = line.split(" ");
                vStopSet.addAll(Arrays.asList(trainingStr));             
            }
    ...
    public static void main(String[] args) throws IOException  {
    	...
    	NB_StopWords NaiveBayes = new NB_StopWords();

        NaiveBayes.read(hamTrainPath);
        NaiveBayes.read(spamTrainPath);
        NaiveBayes.createStopList(stopWordPath);
